...
var appleTrunk,appleLeaves,appleTrunkImg,appleLeavesImg,appleIcon,appleIconImg,keyIcon,keyIconImg;
var apple = false, key=false;
var max,maxImg;
var maxTalk1,maxTalk2;
var exitGate, exitGateImg;
...
function preload(){
  ...
  //tree
  appleTrunkImg=loadImage("./Image/I-Objects/trunk.png");
  appleLeavesImg=loadImage("./Image/Objects/leaves.png");
  appleIconImg=loadImage("./Image/Objects/apple.png");
  keyIconImg=loadImage("./Image/Objects/key.png")
  
  maxImg=loadImage("./Image/NPC/Max.png");
  exitGateImg=loadImage("./Image/I-Objects/wall-door.png");
}
...
function setup() {
  ...
  max=new NPC(20,-80,50,50,maxImg);

  appleTrunk=new iObstacle(300,-100,50,50,appleTrunkImg);

  exitGate=new iObstacle(0,-730,50,50,exitGateImg);
  //player
  ...
  appleLeaves=new Obstacle(300,-186,50,50,appleLeavesImg);
  appleIcon=createSprite(player.x,player.y,50,50);
  appleIcon.addImage(appleIconImg);
  appleIcon.scale=2;
  keyIcon=createSprite(player.x,player.y,50,50);
  keyIcon.addImage(keyIconImg);
  keyIcon.scale=2;
  keyIcon.visible=false;
  maxTalk1=createElement("h3");
  maxTalk1.position(300,800);
  maxTalk1.class("maxtalk1")
  maxTalk2=createElement("h3");
  maxTalk2.position(300,800);
  maxTalk2.class("maxtalk2");
...
function draw() {
  ...
  if(gameState === PLAY){
   ...
   camera.x = player.x;
   camera.y = player.y;
   appleIcon.x=player.x+750;
   appleIcon.y=player.y-400;
   keyIcon.x=player.x+750;
   keyIcon.y=player.y-400;
   ...
   if(area === outSide){
    ...
    if(apple === true){
      appleIcon.visible=true;
    }else{
      appleIcon.visible=false;
    }
   if(area === inHouse1){
  ...
  if(gameState === END){
    ...
    startButton.hide();
      ...
      fill("white");
      textSize(40);
      text(`You pass the gate. You reached the world outside the wall`,-800,-50);
      text(`thankyou for playing.`,-800,0);
      text(`sorry this was not much but it was the first time i made a game on completely on my own `,-800,50);
  }
}
